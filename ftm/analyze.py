...
def _session(proxies: Optional[dict] = None, cookies: Optional[dict] = None) -> requests.Session:
    """Cria uma sessão requests otimizada para contornar bloqueios anti-bot"""
    s = requests.Session()
    ...
    # Cookies básicos para parecer uma sessão real
    basic_cookies = {
        "_ga": f"GA1.2.{random.randint(100000000, 999999999)}.{int(time.time())}",
        "_gid": f"GA1.2.{random.randint(100000000, 999999999)}",
        "cookieconsent_status": "allow"
    }
    s.cookies.update(basic_cookies)
    
    # Requisições resilientes (backoff exponencial)
    retry = Retry(
        total=3,
        connect=3,
        read=3,
    ...
    # proxies/cookies opcionais
    env_proxies = {
        "http": os.environ.get("HTTP_PROXY"),
        "https": os.environ.get("HTTPS_PROXY"),
    }
    s.proxies = proxies or {k: v for k, v in env_proxies.items() if v}
    if cookies:
...
def _fetch_html_like_browser(url: str, sess: requests.Session, referer: Optional[str] = None) -> str:
    """Faz requisição HTTP simulando um navegador real"""
    headers = {}
    if referer:
        headers["Referer"] = referer
    r = sess.get(url, allow_redirects=True, timeout=20, headers=headers)
    r.raise_for_status()
    content = r.content
...
def _cloudscraper_fetch(url: str) -> Optional[str]:
    """Fallback usando cloudscraper para sites com proteção Cloudflare"""
    try:
        import cloudscraper
        ...
        logger.warning(f"Cloudscraper falhou: {str(e)}")
        return None


def _requests_html_fetch(url: str) -> Optional[str]:
    """Fallback usando requests-html para renderizar JavaScript"""
    try:
        from requests_html import HTMLSession
        session = HTMLSession()
        
        # Headers similares aos do navegador
        ...
        logger.warning(f"Requests-html falhou: {str(e)}")
        return None
...
def get_ids(url):
    ...
    try:
        ...
        # Configuração de acesso será feita pelas novas funções auxiliares
        
        # --- tentativa de acesso robusta ---
        sess = _session(proxies=None, cookies=None)
        
        candidates = []
        # 1) esquema original
        candidates.append(url)
        # 2) alternar http/https
        ...
        for idx, cand in enumerate(candidates):
            try:
                # 1ª passada sem referer; se 403, tenta com referer "google"
                try:
                    raw = _fetch_html_like_browser(cand, sess)
                except requests.HTTPError as e:
                    if e.response is not None and e.response.status_code == 403:
                        raw = _fetch_html_like_browser(cand, sess, referer="https://www.google.com/")
                    else:
                        raise
                html = raw
                soup = BeautifulSoup(html, "html.parser")
                url = cand  # Atualiza a URL para a que funcionou
                logger.info(f"Página {cand} acessada com sucesso")
                break
            except Exception as e:
                last_error = e
                logger.warning(f"Falha ao acessar {cand}: {str(e)}")
                time.sleep(random.uniform(0.6, 1.2))  # pequeno backoff e fingerprint diferente
                continue
        
        # 3) fallback com cloudscraper (Cloudflare/WAF)
        if soup is None:
            logger.info("Tentando fallback com cloudscraper...")
            cf_html = _cloudscraper_fetch(candidates[0])
            if cf_html:
                html = cf_html
                soup = BeautifulSoup(html, "html.parser")
                url = candidates[0]
                logger.info(f"Página {candidates[0]} acessada via cloudscraper")
        
        # 4) fallback com requests-html (renderização JavaScript)
        if soup is None:
            logger.info("Tentando fallback com requests-html (JavaScript)...")
            js_html = _requests_html_fetch(candidates[0])
            if js_html:
                html = js_html
                soup = BeautifulSoup(html, "html.parser")
                url = candidates[0]
                logger.info(f"Página {candidates[0]} acessada via requests-html")
        
        if soup is None:
            error_msg = f"Erro ao acessar o site (anti-bot/WAF): {last_error}"
            logger.error(error_msg)
            ids.append(error_msg)
        ...
        # Facebook Pixel (Meta Pixel) - múltiplos padrões
        fb_pixel_patterns = [
            re.compile(r'fbq\(\'init\', \'([0-9]{15,16})\'\)', re.IGNORECASE),
            re.compile(r'facebook_pixel_id["\']?:\s*["\']?([0-9]{15,16})', re.IGNORECASE),
            re.compile(r'_fbp["\']?:\s*["\']?([0-9]{15,16})', re.IGNORECASE)
        ]
        fb_pixel_matches = []
        for pattern in fb_pixel_patterns:
            fb_pixel_matches.extend(pattern.findall(html))
        if fb_pixel_matches:
            ids.append(f"Facebook Pixel (Meta): {', '.join(set(fb_pixel_matches))}")
        
        # TikTok Pixel
        tiktok_patterns = [
            re.compile(r'ttq\.load\(["\']([A-Z0-9]{20})["\']\)', re.IGNORECASE),
            re.compile(r'tiktok_pixel_id["\']?:\s*["\']?([A-Z0-9]{20})', re.IGNORECASE),
            re.compile(r'tt_pixel_id["\']?:\s*["\']?([A-Z0-9]{20})', re.IGNORECASE)
        ]
        tiktok_matches = []
        for pattern in tiktok_patterns:
            tiktok_matches.extend(pattern.findall(html))
        if tiktok_matches:
            ids.append(f"TikTok Pixel: {', '.join(set(tiktok_matches))}")
        
        # LinkedIn Insight Tag
        linkedin_patterns = [
            re.compile(r'_linkedin_partner_id["\']?:\s*["\']?([0-9]{6,8})', re.IGNORECASE),
            re.compile(r'linkedin_insight_id["\']?:\s*["\']?([0-9]{6,8})', re.IGNORECASE),
            re.compile(r'partner_id["\']?:\s*["\']?([0-9]{6,8})', re.IGNORECASE)
        ]
        linkedin_matches = []
        for pattern in linkedin_patterns:
            linkedin_matches.extend(pattern.findall(html))
        if linkedin_matches:
            ids.append(f"LinkedIn Insight Tag: {', '.join(set(linkedin_matches))}")
        
        # Twitter Pixel (X Pixel)
        twitter_patterns = [
            re.compile(r'twq\(\'init\',\s*\'([a-z0-9]{5,10})\'\)', re.IGNORECASE),
            re.compile(r'twitter_pixel_id["\']?:\s*["\']?([a-z0-9]{5,10})', re.IGNORECASE)
        ]
        twitter_matches = []
        for pattern in twitter_patterns:
            twitter_matches.extend(pattern.findall(html))
        if twitter_matches:
            ids.append(f"Twitter Pixel (X): {', '.join(set(twitter_matches))}")
        
        # Pinterest Tag
        pinterest_patterns = [
            re.compile(r'pintrk\(\'load\',\s*\'([0-9]{13})\'\)', re.IGNORECASE),
            re.compile(r'pinterest_tag_id["\']?:\s*["\']?([0-9]{13})', re.IGNORECASE)
        ]
        pinterest_matches = []
        for pattern in pinterest_patterns:
            pinterest_matches.extend(pattern.findall(html))
        if pinterest_matches:
            ids.append(f"Pinterest Tag: {', '.join(set(pinterest_matches))}")
        
        # Snapchat Pixel
        snapchat_patterns = [
            re.compile(r'snaptr\(\'init\',\s*\'([a-z0-9-]{36})\'\)', re.IGNORECASE),
            re.compile(r'snapchat_pixel_id["\']?:\s*["\']?([a-z0-9-]{36})', re.IGNORECASE)
        ]
        snapchat_matches = []
        for pattern in snapchat_patterns:
            snapchat_matches.extend(pattern.findall(html))
        if snapchat_matches:
            ids.append(f"Snapchat Pixel: {', '.join(set(snapchat_matches))}")
        ...
        youtube_patterns = [
            ...
            re.compile(r'youtube_channel_id["\']?:\s*["\']?([A-Za-z0-9_-]{24})', re.IGNORECASE)
        ]
        youtube_matches = []
        for pattern in youtube_patterns:
            youtube_matches.extend(pattern.findall(html))
        if youtube_matches:
            ids.append(f"YouTube Analytics: {', '.join(set(youtube_matches))}")
        
        # Criteo Pixel
        criteo_pattern = re.compile(r'criteo_q\.push\(\["setAccount",\s*([0-9]+)\]\)', re.IGNORECASE)
        criteo_matches = criteo_pattern.findall(html)
        if criteo_matches:
            ids.append(f"Criteo Pixel: {', '.join(set(criteo_matches))}")
        
        # Reddit Pixel
        reddit_pattern = re.compile(r'rdt\(\'init\',\s*\'([a-z0-9]{6,8})\'\)', re.IGNORECASE)
        reddit_matches = reddit_pattern.findall(html)
        if reddit_matches:
            ids.append(f"Reddit Pixel: {', '.join(set(reddit_matches))}")
        
        # Quora Pixel
        quora_pattern = re.compile(r'qp\(\'init\',\s*\'([a-z0-9]{32})\'\)', re.IGNORECASE)
        quora_matches = quora_pattern.findall(html)
        if quora_matches:
            ids.append(f"Quora Pixel: {', '.join(set(quora_matches))}")
